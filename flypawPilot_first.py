...
class FlyPawPilot(StateMachine):
    def __init__(self):
        ...
        self.basestationIP = "192.168.129.2" #other side of the radio link
    ...
    #     startposition = Coordinate(self.CurrentTask.position.lat,self.CurrentTask.position.lon,self.CurrentTask.position.alt)
    #     defaultNextCoord = Coordinate(self.CurrentTask.position.lat,self.CurrentTask.position.lon,self.CurrentTask.position.alt)
    ...
    #     await drone.goto_coordinates(defaultNextCoord)
    #     recover = False
    #     print(drone._waypoint_count, "mathi")
    #     moving = asyncio.ensure_future(drone.goto_coordinates(defaultNextCoord))
    #     while not moving.done() and not recover:
    #         checkForCapture = self.checkPhysicalCapture(drone)
    #         if checkForCapture:
    #             drone._is_cancelled = True
    #             recover=True
    #             moving.cancel()
    #             moving.done()
    #             print(drone._waypoint_count, "tala")
    #             try:
    #                 await moving  # Important: wait for cleanup in goto_coordinates()
    #             except asyncio.CancelledError:
    #                 print("Movement task cancelled successfully.")
    #             await drone.goto_coordinates(defaultNextCoord)
    ...
    #     # Start the task to move to the next waypoint
    #     moving = asyncio.ensure_future(drone.goto_coordinates(defaultNextCoord))
    ...
    #     while not moving.done() and not recover:
    #         checkForCapture = self.checkPhysicalCapture(drone)  # This function checks if capture occurred
    #         if checkForCapture:
    #             print("Capture detected!")
    #             # Capture detected: cancel current movement
    #             drone._is_cancelled = True  # Set cancellation flag
    #             recover = True  # Set recover flag to move to next waypoint
                
    #             # Cancel the current movement task
    #             moving.cancel()
                
    #             # Ensure proper cleanup of the task after cancellation
    #             try:
    #                 await moving  # Wait for the task to finish (catch the cancellation)
    #             except asyncio.CancelledError:
    #                 print("Movement task cancelled successfully.")
                
    #             # After cancellation, re-trigger movement to the next waypoint (next coordinates)
    #             await drone.goto_coordinates(defaultNextCoord)  # You may want to change this to the next waypoint coordinates.
            
    #         await asyncio.sleep(2)  # Sleep for a short time before checking again if needed

    #     # Final state handling after the movement is complete or recovered
    #     print("Movement completed or recovery finished.")
    #     self.WatchDog.ActionComplete(self.CurrentTask)
    #     return "waypoint_entry"  # Proceed to next state after the task is done

    # Working solution do not modify
    # @state(name="flight")
    # async def flight(self, drone: Drone):
    #     print(drone._waypoint_count, "***********************")
    #     logState(self.logfiles['state'], "flight")

    #     defaultNextCoord = Coordinate(
    #         self.CurrentTask.position.lat,
    #         self.CurrentTask.position.lon,
    #         self.CurrentTask.position.alt
    #     )

    #     print("Trying to fly to:", defaultNextCoord)

    #     # Deviation not needed if already at location
    #     if abs(defaultNextCoord.lat - self.currentPosition.lat) < 0.0001 and \
    #     abs(defaultNextCoord.lon - self.currentPosition.lon) < 0.0001:
    #         self.WatchDog.ActionComplete(self.CurrentTask)
    #         return "waypoint_entry"

    #     # Launch movement task
    #     drone._is_cancelled = False
    #     movement_task = asyncio.create_task(drone.goto_coordinates(defaultNextCoord))

    #     while not movement_task.done():
    #         # Check for spoofed deviation
    #         if self.checkPhysicalCapture(drone):
    #             print("!! DEVIATION DETECTED — INITIATING RECOVERY !!")
    #             drone._is_cancelled = True
    #             movement_task.cancel()
    #             try:
    #                 await movement_task
    #             except asyncio.CancelledError:
    #                 print("Movement cancelled due to spoof detection")

    #             # Reroute: try again with correct coordinate
    #             drone._is_cancelled = False
    #             drone._waypoint_count = 1
    #             await drone.goto_coordinates(defaultNextCoord)
    #             break
    ...
    #solution for home landing
    @state(name="flight")
    async def flight(self, drone: Drone):
        logState(self.logfiles['state'], "flight")

        # Define the original task coordinate
        task_coord = Coordinate(
            self.CurrentTask.position.lat,
            self.CurrentTask.position.lon,
            self.CurrentTask.position.alt
        )

        # Define home position — ideally loaded from mission config
        home_coord = Coordinate(35.72733029221068, -78.69617521897719, 50)

        print(f"Flying to task location: {task_coord}")
        drone._is_cancelled = False
        movement_task = asyncio.create_task(drone.goto_coordinates(task_coord))

        while not movement_task.done():
            if self.checkPhysicalCapture(drone) and drone._waypoint_count == 4:
                print("⚠️ GPS spoofing detected! Returning to launch...")
                ...
                try:
                    ...
                    print("Cancelled spoofed movement.")

                # Return to launch
                # drone._is_cancelled = False
                print(f"Returning to start location: {home_coord}")
                await drone.goto_coordinates(home_coord)
...
# flypaw>flypawDrone>drone>flypawPilot>flypawPilot_first.py
